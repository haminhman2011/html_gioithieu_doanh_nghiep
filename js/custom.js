...
// Fact Counter
function update_call(value) {

	jQuery('#support').countTo({
	            from: 0,
	            to: value,
	            speed: 1000,
	          formatter: function (value, options) {
...
jQuery(document).ready(function(){
	var element = jQuery('.feature-service');
	var lis = element.find('.column');
	var maxH;
	for(var i = 0; i < lis.length; i = i + 2){
		// console.log(jQuery(lis[i]).height());
		// console.log(jQuery(lis[i + 1]).height());
		var val1=jQuery(lis[i]).height();
		var val2=jQuery(lis[i+1]).height();
		// console.log(val1);
		// console.log(val2);
		if(val1>val2){maxH=val1}
		else {maxH=val2}
			//console.log(maxH);
		jQuery(lis[i]).css('min-height',maxH);	
		jQuery(lis[i+1]).css('min-height',maxH);	
